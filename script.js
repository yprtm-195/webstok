const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbyzDz-T1b_7TbXtDJQ7zcpSO7vfhgrAeGo8b0Pnd3TU2_2LEHjTH_V36o07ZNiNU1SF/exec';
    ...
    async function executeStokCheck() {
      ...
      cekStokBtn.disabled = true;
      spinner.style.display = 'inline-block';
      document.getElementById('result-card').style.display = 'block';
      document.getElementById('judulStok').textContent = `Stok ${storeInput.split(' - ')[1] || storeInput}`;
      ...
      try {
        ...
        if (!hasAutoRefreshed) {
          ...
          setTimeout(executeStokCheck, 750);
        } else {
          currentProductData = finalProductList;
          cekStokBtn.disabled = false;
          spinner.style.display = 'none';

          // Collapse the "Pilih Toko" section
          const collapsePilihTokoElement = document.getElementById('collapsePilihToko');
          if (collapsePilihTokoElement) {
            const bsCollapse = new bootstrap.Collapse(collapsePilihTokoElement, {
              toggle: false
            });
            bsCollapse.hide();
          }
    ...
    function downloadAsXlsx() {
      ...
      const fileName = `stok_${storeName}_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
    ...
    function downloadAsCsv() {
      ...
      const today = new Date();
      const fileName = `stok_${String(today.getDate()).padStart(2, '0')}${String(today.getMonth() + 1).padStart(2, '0')}${today.getFullYear()}.csv`;
    ...
    document.getElementById('cekStok').addEventListener('click', () => {
